...
public:
    ...
    map<uint32_t, uint32_t> PrPtr;      // Active CacheSet pointer  {AddrSET,PrPTR[AddrSet]}
    map<uint32_t, uint32_t> PsPtr;      // Historical CacheSet pointer  {AddrSET,PsPTR[AddrSet]}
    map<uint32_t, uint32_t> fill_count; // Track W fills per AddrSet  {AddrSET,cachefills to a PrPTR[AddrSet]}
    ...
    // constructor
    CACHE(string v1, uint32_t v2, int v3, uint32_t v4, uint32_t v5, uint32_t v6, uint32_t v7, uint32_t v8, uint8_t v9 = 0)
        : NAME(v1), NUM_SET(v2), NUM_WAY(v3), NUM_LINE(v4), WQ_SIZE(v5), RQ_SIZE(v6), PQ_SIZE(v7), MSHR_SIZE(v8), cache_type(v9)
    {
        ...
        if (cache_type == IS_LLC)
        {
            ...
            vector<int> Random_taken(NUM_SET);

            for (int i = 0; i < freelist_size; i++)
            {
                int random_number = (std::rand() % NUM_SET);
                while (Random_taken[random_number])
                {
                    random_number = (std::rand() % NUM_SET);
                }

                freelist[i] = random_number;
                Random_taken[random_number] = 1;
            }
